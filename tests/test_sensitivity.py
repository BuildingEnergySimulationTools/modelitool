...
@pytest.fixture()
def expected_res():
    common_index = pd.date_range("2009-01-01 00:00:00", freq="s", periods=3)
...
class TestSensitivity:
    ...
    def test_run_simulation(self, simul, sa_param_config, expected_res):
        test_sample = np.array([[1.0, 1.0, 1.0], [2.0, 2.0, 2.0], [3.0, 3.0, 3.0]])
    ...
    def test_compute_aggregated(self, simul, expected_res, sa_param_config):
        ...
        res1 = sa_object._compute_aggregated(
            aggregation_method=np.mean, indicator="res1.showNumber"
        )

        res2 = sa_object._compute_aggregated(
            aggregation_method=np.mean, indicator="res2.showNumber"
        )

        res_mean = sa_object._compute_aggregated(
            aggregation_method=mean_error, indicator="res1.showNumber", ref=ref_agg
        )

        res_freq = sa_object._compute_aggregated(
            aggregation_method=np.mean, indicator="res1.showNumber", freq="2S"
        )
        ...
        np.testing.assert_array_almost_equal(
            res1, np.array([5.88213201, 8.15192598, 1.42359607]), decimal=6
        )

        np.testing.assert_array_almost_equal(
            res2, np.array([4.802573569, 12.31778589, 5.956054618]), decimal=6
        )

        np.testing.assert_array_almost_equal(
            res_mean, np.array([-0.729580657, -2.999374628, 3.728955285]), decimal=6
        )
    ...
    def test_analyse(self, simul):
        ...
        df_to_compare = pd.DataFrame(
            {key: res["S1"] for key, res in sa.sensitivity_dynamic_results.items()}
        ).T
